...
class TemporalGeometryTest(unittest.TestCase):
    ...
    def test_ego_boundary_width_shrink_is_symmetric(self):
        items = [{"valid": True, "left_xy": [[5.0, -2.0], [10.0, -2.0]], "right_xy": [[5.0, 2.0], [10.0, 2.0]]}]
        fused, _ = fuse_ego_boundary_keypoints(items, width_shrink=0.75)
        self.assertAlmostEqual(fused[0]["left_xy"][0][1], -1.5, places=6)
        self.assertAlmostEqual(fused[0]["right_xy"][0][1], 1.5, places=6)
