...
class ContinuousMotionTest(unittest.TestCase):
    ...
    def test_speed_gate_does_not_remove_shape_signals(self) -> None:
        times = [0.5, 1.0, 1.5, 2.0]
        value = decoder([4.0] * 4, curvature=0.02)
        for row in value["speed_support"]:
            row["status"] = "uncertain"
        imagined = image_motion_profile(
            value,
            times,
            initial_speed_mps=4.0,
            shape_status_by_interval=["usable"] * 4,
            shape_observability_by_interval=[0.4] * 4,
        )
        action = trajectory_to_motion_profile(value["trajectory"], times, initial_speed_mps=4.0)
        result = compare_motion_profiles(imagined, action)
        assert result["coverage"] == 1.0
        assert result["metrics"]["lateral_speed_mps"]["count"] == 4
        assert result["metrics"]["yaw_rate_radps"]["count"] == 4
        assert result["metrics"]["curvature_1pm"]["count"] == 4
        assert result["metrics"]["speed_mps"]["count"] == 0
        assert result["speed_posterior"]["count"] == 0

    def test_dead_flow_can_be_reported_as_categorical_stop(self) -> None:
        value = decoder([0.05] * 4)
        for row in value["speed_support"]:
            row["status"] = "abstain"
            row["observability"] = 0.02
        profile = image_motion_profile(
            value,
            [0.5, 1.0, 1.5, 2.0],
            initial_speed_mps=0.0,
            flow_status_by_interval=["low_flow_magnitude"] * 4,
        )
        self.assertEqual(profile["stationary_state"]["state"], "stopped")
        self.assertGreaterEqual(profile["stationary_state"]["confidence"], 0.70)
        self.assertEqual(classify_stationary_state(profile["rows"], initial_speed_mps=0.0)["state"], "stopped")
