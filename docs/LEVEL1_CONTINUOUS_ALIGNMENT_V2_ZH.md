...
短时域样本仍保留 `relative`（终点前向位移）作为兼容模式；8 帧/4 秒正式协议的
主 pose 归一化改为 `arc_relative`：每条 profile 独立用二维轨迹弧长归一化。长时域
转弯时终点前向位移可能接近零，`|s(T)|` 会成为病态分母；弧长对这类样本更稳定，
同时不要求单目图像恢复绝对米制尺度，也不把 action 的尺度泄漏回图像分支。`relative`
和 `scale_free` 保留为敏感性与兼容性诊断，`metric` 只作为绝对距离诊断。
...
在 4 帧结果上，`x_end` 仍略优；但在目标 8 帧/4 秒协议中，结论反转。25 个非重叠样本的均值如下：

| 变体 | 逐点 pose MAE | Fréchet | 受限 DTW |
|---|---:|---:|---:|
| 4 帧 `x_end` | 0.120 | 0.440 | 0.295 |
| 4 帧 `arc` | 0.151 | 0.505 | 0.347 |
| 8 帧 `x_end` | 0.616 | 2.472 | 1.481 |
| 8 帧 `arc` | 0.092 | 0.355 | 0.215 |

因此 8 帧/4 秒主协议采用 `arc_relative`，`x_end` 只作敏感性报告。Fréchet 和受限 DTW 均保留时间顺序：
