...
1. 纵向解码改为估计相对速度变化与加速度残差，而不是重新估计绝对速度尺度。
2. 历史状态只提供 `t=0` 的米制 anchor；future 图像必须负责之后的变化量。
3. 在独立 calibration split 上拟合残差尺度和速度区间，不在 test split 选择融合权重。
4. 使用 NAVSIM 与 Waymo 的真实 `8 帧/4 秒` WAM future/action 样本重新运行三项增量门槛。
5. 纵向分量未通过前，不构造 Level 1 总分，也不进入事件或 Level 2。
