...
def _metrics(prediction: np.ndarray, target: np.ndarray, inflation: float = 1.0) -> dict[str, float]:
...
def main() -> None:
    ...
    # Fit one conservative scalar interval inflation on calibration only.
    train_pred = denormalize_support(torch.from_numpy(train_prediction)).numpy()
    train_truth = denormalize_support(torch.from_numpy(train_y)).numpy()
    required = np.abs(train_truth[..., [0, 2, 4]] - train_pred[..., [0, 2, 4]]) + train_truth[..., [1, 3, 5]]
    available = np.maximum(train_pred[..., [1, 3, 5]], 1e-4)
    inflation = float(np.quantile(np.max(required / available, axis=-1), 0.90))
    ...
    report = {
        ...
        "calibration_interval_inflation": inflation,
        "train_raw": _metrics(train_prediction, train_y),
        "holdout_raw": _metrics(holdout_prediction, holdout_y),
        "train_calibrated": _metrics(train_prediction, train_y, inflation),
        "holdout_calibrated": _metrics(holdout_prediction, holdout_y, inflation),
