...
def _pair_score(first: dict[str, Any], second: dict[str, Any]) -> dict[str, Any]:
    ...
    own_better = own < cross
    shape_cc = float(np.mean(own_better))
    margin = float(np.mean(cross - own))
...
def main() -> None:
    ...
    shape_cc = [row["shape_counterfactual_consistency"] for row in results]
    margins = [row["shape_cc_margin"] for row in results]
    ...
    summary = {
        ...
        "by_condition_family": {
            family: {
                "pairs": len(values),
                "mean_shape_counterfactual_consistency": float(np.mean([v["shape_counterfactual_consistency"] for v in values])),
                "mean_shape_cc_margin": float(np.mean([v["shape_cc_margin"] for v in values])),
        ...
        "errors": errors,
        "results": results,
        "interpretation": "Image-plane CC evaluates direction/shape response only; it cannot validate absolute metric speed without camera calibration or an independent ego-state reference.",
    }
